...
var decimalRangeStorage =localStorage.getItem('decimalRange');
if (decimalRangeStorage == null || decimalRangeStorage == 'undefined' ) { decimalRangeStorage = 0;}
const numberFormat = (number,digits = parseInt(decimalRangeStorage)) => new Intl.NumberFormat('en-US',{maximumFractionDigits: digits}).format(number);

$(".logout_ppal").on("click",function(e){sessionStorage.clear();});

const semaforo = (id, porcentaje) => {
	if(porcentaje >= 98) {
		$(`#${id}`).removeClass('pastnar');
		$(`#${id}`).removeClass('pastama');
		$(`#${id}`).addClass('pastver');
	}
	if(porcentaje < 98 && porcentaje > 90) {
		$(`#${id}`).removeClass('pastver');
		$(`#${id}`).removeClass('pastnar');
		$(`#${id}`).addClass('pastama');
	}
	if(porcentaje < 90) {
...
const semaforoDOM = (elemento, porcentaje) => {
	if(porcentaje >= 98) elemento.className = "pastver";
	if(porcentaje < 98 && porcentaje > 90) elemento.className = "pastama";
	if(porcentaje < 90) elemento.className = "pastnar";
}

const downloadFile = (filePath,fileName) => {
	//$("#msg-file-status").remove();
	//$("#download_excel").remove();
	let input_file = `<p id="msg-file-status"><strong>El archivo esta listo para la descarga: </strong></p><a class="btn btn-success" id="download_excel" href="./${filePath}" download="${fileName}">Descargar</a>`;
	$('#dwnf-msg').html(input_file);
}

const createFile = (indicador,month,options = false) => {
	$("#dwnf").css("display","block");
	$("#container_loader_file").css("display","block");
	$("#dwnf-msg").empty();
	if(options !== false) opt = options;
	else opt = {indicador,month};
	$.post('./php/download_excel.php',opt,(rs) => {
		if(rs.success != undefined) {
			nameFile = rs.name != undefined ? rs.name : rs.nameFile;	
			downloadFile(`./${rs.nameFile}`,nameFile);
		}
		else $("#dwnf").html("<h4>Ocurrio un error al generar el archivo</h4>");
		$("#container_loader_file").css("display","none");
	});
}

const downloadFileByHtml = (indicador,contenido) => {
	$("#dwnf").css("display","block");
	$("#container_loader_file").css("display","block");
	$('#dwnf-msg').empty();
	$.post('./php/download_excel.php',{indicador,contenido,byHtml:1},(rs) => {
		console.log(rs);
		if(rs.success != undefined) downloadFile(`./${rs.nameFile}`,rs.nameFile);	
		else $("#msg-file-download").html("<h4>Ocurrio un error al generar el archivo</h4>");
		$("#container_loader_file").css("display","none");
	});
